...
- 아래로 스크롤해 **백업** 섹션 확인
- 다운로드 옵션:
  - **데이터베이스**: `invoice.db` 파일 다운로드
  - **업로드 파일**: 모든 업로드 파일 ZIP으로 다운로드
  - **전체 백업**: 데이터베이스 + 업로드 파일 ZIP으로 다운로드
- **복원** 섹션에서 백업 파일 업로드 가능
- 복원 전 확인 필요 (기존 데이터 덮어쓰기)
- 데이터베이스 복원 후 자동 페이지 새로고침
...
### 백업 & 복원
- **데이터베이스 백업**: SQLite 데이터베이스 파일 다운로드
- **업로드 파일 백업**: 모든 업로드 파일 (로고, 서명) ZIP으로 다운로드
- **전체 백업**: 데이터베이스 + 업로드 파일 ZIP으로 다운로드
- **복원**: 백업 파일 업로드해 데이터 복원
- 모든 작업은 대시보드에서 가능
- 복원 전 자동 안전 백업 생성
